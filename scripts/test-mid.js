...
function extractBookData(filePath) {
  try {
    ...
    return {
      ...
      添加时间: formatDate(frontmatter.添加时间 || frontmatter.addDate || ''),
      开始时间: formatDate(frontmatter.开始时间 || frontmatter.startDate || ''),
      结束阅读: formatDate(frontmatter.结束阅读 || frontmatter.finishDate || ''),
