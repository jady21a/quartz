...
2025-11-21
我发现一个命令可以一次执行就完成同步两个本地仓库,并且发布,然后设置定时自动运行该命令就可以了,最终选择了方案三,第一次尝试的时候死活不成功,这次很快就成功了,果然过于死磕并不是好事
