...
这是 **Cloudflare Pages 的一个已知问题**。当 sitemap.xml 文件被当作静态资源托管时，Cloudflare Pages 可能没有正确设置 `Content-Type` 响应头，导致 Google bot 无法正确解析,添加 _headers 文件解决
在你的项目根目录创建一个 `_headers` 文件：
```
